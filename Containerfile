...
# A toolchain for the duration of this step only.
#
# pi-web 0.9.0 added node-pty, whose npm tarball ships prebuilt bindings for
# darwin-arm64, darwin-x64, win32-arm64 and win32-x64 — and no Linux target at
# all. Its install hook is `node scripts/prebuild.js || node-gyp rebuild`, so on
# every Linux architecture, x86_64 included, it falls through to compiling from
# source. Without make and a C++ compiler the build dies with:
#
#     gyp ERR! stack Error: not found: make
#
# make and g++ are purged again in the same layer so the runtime image does not
# carry them. libstdc++6 and libgcc-s1 are marked manual first, or --auto-remove
# takes the runtime libraries the compiled .node binding links against. Verify
# after any change with:
#
#     node -e 'require("/usr/lib/node_modules/@agegr/pi-web/node_modules/node-pty")'
#
# This is a no-op for PI_WEB_VERSION values with no native dependency; every
# release up to and including 0.8.11 builds without it.
RUN set -eux; \
    apt-get update; \
    apt-get install -y --no-install-recommends make g++; \
    npm install -g --omit=dev "@agegr/pi-web@${PI_WEB_VERSION}"; \
    apt-mark manual libstdc++6 libgcc-s1; \
    apt-get purge -y --auto-remove make g++; \
    apt-get clean; \
    rm -rf /var/lib/apt/lists/* /tmp/npm-cache
