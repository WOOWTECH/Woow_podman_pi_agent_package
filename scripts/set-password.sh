...
# nginx drops request workers to its image's nginx group. A host-owned 0600
# bind mount is readable by the rootless master but not those workers. Create
# the file privately, map its group through the rootless user namespace, then
# atomically publish it as container root:nginx 0640.
install_htpasswd() {
  local content="${1-}" tmp nginx_gid
  umask 077
  tmp=$(mktemp "${CONFIG_DIR}/.htpasswd.XXXXXX")
  (
    trap 'rm -f -- "$tmp"' EXIT
    printf '%s' "${content}" >"${tmp}"
    nginx_gid=$(podman run --rm --entrypoint /bin/sh "${NGINX_IMAGE}" -ec 'id -g nginx')
    case "${nginx_gid}" in *[!0-9]*|'') die "could not resolve nginx worker gid" ;; esac
    chmod 0640 "${tmp}"
    podman unshare chown "0:${nginx_gid}" "${tmp}"
    mv -f -- "${tmp}" "${HTPASSWD}"
    trap - EXIT
  )
}
...
case "${MODE}" in
  generate)
    ...
    PASSWORD=$(LC_ALL=C od -An -N18 -tx1 /dev/urandom | tr -d ' \n' | cut -c1-24)
    GENERATED=1
    ;;
  stdin)
...
# was.
SALT=$(LC_ALL=C od -An -N8 -tx1 /dev/urandom | tr -d ' \n')
...
# --- write --------------------------------------------------------------------
# Create with the right container-visible owner/mode before publishing it. A
# umask-dependent or worker-unreadable moment would either expose the hash or
# make every valid credential fail closed with an nginx permission error.
install_htpasswd "${USER_NAME}:${HASH}"
