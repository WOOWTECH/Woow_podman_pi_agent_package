...
> **這套部署前面沒有任何身分驗證。** 只要能連到發佈出去的連接埠，就等於拿到一個有 `bash` 工具、以你的帳號身分執行的 coding agent。在把它開放到可信任區網以外之前，請先讀[安全性現況](#安全性現況)。
...
# 建置。VIDEO_TOOLS=0 會產生約 700MB、不含影片工具鏈的映像。
...
rootfs/usr/local/bin/
  ...
  pi-web-start.sh          進入點：權限、skills 橋接、時區、影片工具啟動
...
```bash
# 1. 結構面 — 不呼叫模型、零成本。檢查執行環境、持久化配置、
#    HTTP 介面、CJK 回歸測試與影片工具鏈。
...
兩套測試與 k3s 版本逐項對齊：相同的檢查項目、相同的順序。因此兩輪之間的差異可以被判定為移植缺陷，而不是量測方式不同造成的。
...
**這套部署做得好的地方。** 它是 rootless 的，agent 的 `bash` 工具以一般主機使用者身分執行，而不是節點 root。有加上 `NoNewPrivileges`。pi-web 本身不發佈任何主機連接埠——唯一的監聽者是 nginx，這代表 `/api/models-config`（**未遮蔽**地回傳供應商金鑰，且不需驗證）除了經由 proxy 之外無法觸及。憑證檔權限為 `600`。

**沒有做到的地方。** 任何位置都沒有身分驗證。上游 pi-web 沒有路徑限制、沒有核可關卡、也沒有 `canUseTool` hook——agent 可以讀寫容器使用者能觸及的任何位置，並執行任何指令。這些是上游的性質，打包方式再怎麼調整都無法解決。

**所以。** 請把 30142 埠視同直接發放 shell。若要超出可信任區網使用，前面必須放一層有身分驗證的 proxy——這台 Podman 主機既有的 Cloudflare Tunnel 加 Cloudflare Access 就是預期路徑，這也正是本套件刻意不自帶 tunnel 的原因。若只要本機可用，把 `quadlet/nginx.container` 的 `PublishPort` 改成 `127.0.0.1:30142:30142`。
